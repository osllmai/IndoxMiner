...
class IndoxObjectDetection:
    def __init__(self, model_name, device="cuda", **kwargs):
        ...
        if self.model_name == "kosmos2":
            ...
            self.model = YOLOv10Model(model_path=kwargs.get("model_path", "yolo10n.pt"), device=device)
    ...
    def _initialize_yolox(self, kwargs):
        ...
        self.model = YOLOXModel(exp_file=exp_file, model_path=model_path, device=self.device)
    ...
    def _run_model(self, image_path, **kwargs):
        if self.model_name == "kosmos2":
            ...
            results = self.model.detect_objects(image, score_threshold=kwargs.get("score_threshold", 0.7))
            self.model.visualize_results(image, results)
        elif self.model_name == "detrclip":
            image = self.model.load_image(image_path)
            results = self.model.detect_objects(image, threshold=kwargs.get("threshold", 0.5))
            self.model.visualize_results(image, results)
        elif self.model_name == "sam2":
