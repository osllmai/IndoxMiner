...
class Extractor:
    ...
    #         data = {"items": [data]}
    #     else:
    #         # Separate the list-like fields (potential item lists) from other common fields
    #         items_field_name = None
    #         items = []
    #         common_fields = {}

    #         for key, value in data.items():
    #             # Detect list of dictionaries, assuming it's the items section, but flexible with the name
    #             if isinstance(value, list) and all(
    #                 isinstance(item, dict) for item in value
    #             ):
    #                 items_field_name = key  # Keep track of the name of the items list
    #                 items = value  # Store the list as-is to avoid flattening
    #             elif not isinstance(value, (list, dict)):
    #                 # Collect fields that are not lists or dictionaries as common fields
    #                 common_fields[key] = value

    #         # If we found an items section, keep it intact under its original name
